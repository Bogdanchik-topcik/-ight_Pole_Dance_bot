...
💜17.00 HANDSTAND (Стойки на руках) |ВС - 17:00
💜STRIP| СР 19:00, ВС 18:00
💜CHOREO| СР 20:00, ВС 12:00
💜SALSATION| ПН, ЧТ - 19:00'''))
...
async def prise(call: CallbackQuery, state: FSMContext):
    ...
    new_mesidDB(await call.message.answer_photo('AgACAgIAAxkBAAIBh2bQ-HhXPc4pBAkKVuMdHmoak8zUAAJx5jEbrIWISikU3FVlWShvAQADAgADeAADNQQ', 
    ...
    new_mesidDB(await call.message.answer_photo('AgACAgIAAxkBAAIBiGbQ-NW_O8gcm6c6l2DyDMSkpKlOAAJy5jEbrIWISro6ZsdyexxjAQADAgADeAADNQQ'))
    new_mesidDB(await call.message.answer("<b>Аренда зала:</b>", parse_mode='HTML'))
    new_mesidDB(await call.message.answer_photo('AgACAgIAAxkBAAIBiWbQ-SGzgc48IGDUdu-QnUEquQS-AAJ05jEbrIWISvbJL-z_XrE_AQADAgADeAADNQQ',
...
async def prise(call: CallbackQuery, state: FSMContext):
    ...
    caption='Это танец на пилоне, который в равной мере должен включать в себя владение трюковым пилоном и хореографией. Трюки и элементы должны быть подобранны и наложены на музыку и составлять единый образ с танцевальной составляющей. Соотношение трюков и хореографии варьируется от 50/50 до 70/30.'))
    ...
    caption="Занятия направлены на изучение базовых движений, элементов, методов работы на пилоне. Люди без спортивной подготовки научатся управлять своим телом, готовить его к пилону, а имеющие спортивный опыт освоят основные принципы работы на снаряде. В группе продолжающих изучают элементы более сложного уровня."))
    ...
    caption="Это женственность, грация и свобода от всего, что осталось за пределами зала. Приглушенный свет и глубокая музыка позволяют раствориться в танце и следовать за своими движениями. Раскрыть свою женскую энергетику, наслаждаться собой в отражении - это именно то, что нужно каждой женщине. "))
...
caption='''Это комплекс упражнений, направленный на растяжку связок, мышц и сухожилий. 
...
caption='Strip - не просто танец на высоких каблуках, это невероятно многогранный стиль танца. Он таит в себе почти неограниченные возможности: это игра с мельчайшими музыкальными акцентами, это всегда новый образ и новая история вашей души. Насыщенность, чёткость, лаконичность - всё это Strip.'))
...
caption='Salsation® - это увлекательное сочетание различных танцевальных направлений и музыкальных ритмов. Все хореографии направления разработаны на основе трех главных принципов - сочетание лиричности, музыкальности и функциональности.'))
...
caption='Авторская хореография, совмещающая в себе несколько танцевальных направлений. Танец отражает то, как хореограф видит музыку. Автор передает настроение с помощью всех доступных ему танцевальных техник, соединяя их в единую гармоничную композицию. Если вам хочется разнообразия в танце, и вы за эксперименты - Choreo определенно для вас!'))
...
caption=''' Стойка на руках одна из самых сложных и в тоже время необычный и эффектный трюк. Она способствует укреплению мышц плечевого пояса и спины, улучшает работу вестибулярного аппарата, укрепляет психоэмоциональное состояние и повышает самооценку.
