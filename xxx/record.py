...
async def records(call: Message, state: FSMContext): 
    ...
    ed.new_messID(await call.message.answer("Введите имя"))
    await call.answer()
...
async def name(mess: Message, state: FSMContext):
    ...
    ed.new_messID(await mess.answer("Введите свой номер телефона"))
...
async def phone(mess: Message, state: FSMContext):
    ...
    ed.new_messID(await mess.answer("Выберете направление:", reply_markup=Ikb1))
...
async def diction(call: CallbackQuery, state: FSMContext):
    ...
    ed.new_messID(await call.message.answer('Напишите комментарий или вопрос. если хотите пропустить, то нажмите "пропустить" ', reply_markup=Rkb1))
    await call.answer()
...
async def comment(mess: Message, state: FSMContext):
    ...
    #id = '5465180498'  # my
    id = '522687117'  # julia
    await bot.send_message(chat_id=id, text=f"{data['name']} записалась на занятие.\nТелефон: {data['phone']}\nНаправление: {data['direction']}")
    if data['comment'] != 'пропустить':
        await bot.send_message(chat_id=id, text=f"{data['comment']}")

    ed.new_messID(await mess.answer("Принято 👍", reply_markup=ReplyKeyboardRemove()))
